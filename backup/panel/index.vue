  ...
  <LayoutPanel>
    <v-container class="w-100">
      ...
      <div class="d-flex pa-10 flex-column w-100 justify-space-between align-center">
        <CardSubmission  :actual_title="EditionStore?.currentEdition?.theme || 'Tema não disponível'" :work="SubmissionVerify?.work" :work_status="SubmissionVerify?.status"/>
      </div>
      <div class="d-flex justify-space-between align-center text-h6">
      ...
      <div >
        <div v-if="verifySubmitWork.length > 0">
          <CardSubmission v-for="works in verifySubmitWork" :key="works.id" :work="works.edition.final_submission_date" :work_status="2" :actual_title="works.edition.theme || 'Tema não disponível'"/>
        </div>
