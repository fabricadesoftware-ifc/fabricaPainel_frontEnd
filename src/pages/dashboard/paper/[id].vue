  ...
  <!-- <LayoutSteps>
    <v-card class="border-md w-100 pa-8" rounded="xl" variant="outlined">
      <header class="d-flex justify-space-between">
        <h1>{{ paper.id }} - {{ paper.name }}</h1>
        <div>
          <v-chip v-for="(value, key) in categories" :key="key" class="mr-2 my-4" :color="getColor(key)" outlined>
            {{ value }}
          </v-chip>
        </div>
      </header>
      ...
      <v-expansion-panels v-model="panel" class="rounded-0 overflow-hidden" multiple variant="accordion">
        <v-expansion-panel value="team">
          ...
          <v-expansion-panel-text>
            <ul class="font-weight-bold">
              <li class="d-block">
                {{ team.teamRep }}
                <span class="d-inline font-weight-light">( Líder )</span>
              </li>
        ...
        <v-expansion-panel value="evaluation">
          ...
          <v-expansion-panel-text><v-row>
              <v-col cols="6">
                <p>Orientadores</p>
                <v-chip class="mr-2 my-4" color="primary" outlined> Professor 1 </v-chip>
                <p>Co-Orientadores</p>
                <v-chip class="mr-2 my-4" color="primary" outlined> Professor 1 </v-chip>
              </v-col>
              <v-col cols="6">
                <p>Avaliadores</p>
                <v-chip class="mr-2 my-4" color="primary" outlined> Professor 2 </v-chip>
                <v-chip class="mr-2 my-4" color="primary" outlined> Professor 2 </v-chip>
                <v-chip class="mr-2 my-4" color="primary" outlined> Professor 2 </v-chip>
              </v-col>
            </v-row>
        ...
        <div class="pa-4 text-center">
          <v-dialog scrollable width="auto">
            <template #activator="{ props: activatorProps }">
              <v-btn color="info" rounded="xl" variant="flat" v-bind="activatorProps">Registar Avaliadores</v-btn>
            </template>

<template #default="{ isActive }">
              <v-card prepend-icon="mdi-account-group" title="Selecione os Avaliadores">
                <v-divider class="mt-3" />
                <v-card-text class="px-4">
                  <v-autocomplete chips clearable :items="[
                    'Professor 1',
                    'Professor 2',
                    'Professor 3',
                    'Professor 4',
                    'Professor 5',
                  ]" label="Temas Transversais" multiple rounded="xl" variant="outlined" />
                </v-card-text>
                <v-card-actions>
                  <v-btn color="red" rounded="xl" text="Fechar" @click="isActive.value = false" />
                  <v-btn color="primary" rounded="xl" text="Confirmar" @click="isActive.value = false" />
                </v-card-actions>
              </v-card>
            </template>
</v-dialog>
...
// const id = ref("");
// const panel = ref(["team", "abstract", "evaluation"]);
