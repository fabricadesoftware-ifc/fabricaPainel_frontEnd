...
<template>
  <LayoutPanel v-if="workStore.currentWork && isLoaded">
    <v-container class="w-100">
      <v-btn href="/panel/works" style="box-shadow: none;" class="mb-2 text-blue"> <v-icon
      ...
      <v-fade-transition appear>
        <div class="d-flex flex-column ga-10">
          ...
          <div class="d-flex flex-column ga-3 flex-wrap w-100"
          ...
          <MembersContainer
            :attribute="width <= 780 ? '' : ['ADVISOR'].includes(resolveUserFunction(workStore?.currentWork, authStore?.user)) ? 'Nota Individual Final' : ['STUDENT'].includes(resolveUserFunction(workStore?.currentWork, authStore?.user)) ? 'Nota Individual' : ''">
            <MembersCard
          ...
          <MembersContainer title="Colaboradores do Trabalho" :attribute="width > 780 ? 'Status do Convite' : ''">
            <MembersCard v-for="(collaborator, index) in workStore.currentWork.work_collaborator"
              ...
              :status="collaborator.status" :user_id="authStore.user.id"
              :work_advisor="workStore?.currentWork?.advisor ?? null" :key="index" />
          </MembersContainer>
        </div>
      </v-fade-transition>

      <acceptance-work v-if="
        acceptanceStore.state.isCollaborator &&
        acceptanceStore.state.collaboratorStatus === 1 &&
        uptadeWorkStatus != 4
      " :work="workStore.currentWork" />

      <AcceptanceAdvisorWork v-if="
