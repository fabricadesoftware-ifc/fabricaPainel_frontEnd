...
onMounted(async () => {
  await fetchEditions().then(() => {
    ...
    // @ts-ignore
    edition.value = state.editions.find(
      (ed: any) => ed.id === Number(id.value),
    );
  });
});
//@ts-ignore
const able_to_download_grades = computed(() => {
  return (
    date > new Date(edition.value?.final_event_date ?? "2100-01-01") &&
    ["TEACHER", "ADMIN"].includes(authStore.user.user_type)
  );
});
...
<template>
  <LayoutPanel>
    <v-container>
      ...
      >
        <v-icon icon="mdi-arrow-left mr-1"></v-icon> Voltar</v-btn
      >
          ...
          <div class="d-flex align-center justify-space-between ga-15 w-50">
            <div class="d-flex align-center ga-10">
              ...
              <VChip
                v-else-if="
                  !edition?.is_active &&
                  upcomingEdition.some((up: IEdition) => up.id === edition?.id)
                "
                class="bg-yellow"
                >Em Breve</VChip
              >
              <VChip
                v-else
                class="bg-red d-flex justify-center align-center"
                pill
                style="width: 120px"
                >Encerrado</VChip
              >
            ...
            <v-btn
              @click="studentAssesment.fetchAssessmentReport(edition.year)"
              v-if="able_to_download_grades"
              color="blue"
            >
              <v-icon>mdi-download</v-icon>
              <p class="ml-2">Baixar Relatório de Notas</p></v-btn
            >
          </div>
          <v-row class="ga-10">
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <span class="text-primary font-weight-bold"
                  >LIMITES DE CARGO</span
                >
                <VChip
                  class="bg-green d-flex justify-center align-center"
                  pill
                  style="width: 120px; height: 30px"
                  >Professor</VChip
                >
