<template>
  <section
    class="py-16 my-16 bg-grey-darken-4"
    :class="{ 'pt-0 pb-0': screenWidth < 700 }"
  >
    <div class="py-16">
        ...
        >
          ...
          >
            ...
            <div class="h-100 d-flex flex-column justify-space-between pa-6">
              <p v-if="edition.is_active" class="text-info">Aberta</p>
              ...
              <p class="text-grey">
                O tema desta Edição é "{{ edition.theme }}". A data inicial de
                submissões é
                {{
                  date.format(
                ...
                e a data final é
                {{
                  date.format(edition.final_submission_date, "keyboardDate")
                }}.
              </p>
      ...
      <v-expand-transition>
        <v-sheet v-if="model != null" class="bg-grey-darken-4">
          <v-container class="w-lg-75 pb-0" fluid>
            ...
            <TitleH1
              color="text-primary"
              :text="state.editions[model]?.edition_name"
            />
            <v-row>
              <v-col cols="12" md="6">
                <img
              ...
              <v-col class="text-grey text-start" cols="12" md="6">
                <p class="pb-4 text-h6">
                  ...
                  <span class="text-white font-weight-bold">
                    {{ state.editions[model]?.workload }} horas</span
                  >
                </p>
                <p>
                  Data de Submissão:
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.initial_submission_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
                  ...
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.final_submission_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
                ...
                <p class="py-2">
                  ...
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.initial_advisor_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
                  ...
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.final_advisor_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
                ...
                <p>
                  ...
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.initial_evaluators_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
                  ...
                  <span class="text-white font-weight-bold">
                    {{
                      date.format(
                        state.editions[model]?.final_evaluators_date,
                        "keyboardDate",
                      )
                    }}
                  </span>
              ...
              <v-col class="pt-8">
                <v-btn
                  ...
                  text
                  @click="
                    state.editions[model]?.is_active
                      ? router.push('/panel')
                      : router.push(
                          `/panel/editions/view/${state.editions[model]?.id}`,
                        )
                  "
                >
                  <div v-if="state.editions[model]?.is_active">
                    Inscrever-se
                  </div>
                  <div v-else>Ir para página da Edição</div>
                </v-btn>
              </v-col>
            </v-row>
          </v-container>
