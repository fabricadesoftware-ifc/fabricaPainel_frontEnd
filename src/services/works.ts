...
class WorkService {
  ...
  async getAdminClassProposalReportData(params: { edition?: string | number | null; class?: string | number | null }) {
    try {
      const { data } = await api.get("/work/admin-class-report-data/", { params });
      return data;
    } catch (error) {
      this.handleError(error, "fetch admin class report data");
    }
  }
  ...
  async downloadAdminClassProposalReport(params: { edition?: string | number | null; class?: string | number | null }) {
    try {
      const response = await api.get("/work/admin-class-report/", {
        params,
        responseType: "blob",
      });
      const disposition = response.headers["content-disposition"] || "";
      const filenameMatch = disposition.match(/filename="?([^";]+)"?/i);
      const filename = filenameMatch?.[1] || "relatorio_propostas_turma.pdf";
