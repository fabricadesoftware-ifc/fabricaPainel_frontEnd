...
export const removeWork = async (
  id: String,
  store: StoreWithRemoveWork,
  token: String
) => {
  await store.removeWork(id, token);
};

export const aproveWork = async (
  verification_token: String,
  store: StoreWithAproveWork
) => {
...
export const rejectWork = async (
  verification_token: String,
  store: StoreWithRejectWork
) => {
  await store.rejectWork(verification_token);
};

export const validate_user_function = (
  ...
  if (user_function === "EVALUATOR" || user_function === "ADVISOR") {
    if (work_status == 2) {
      ...
      (user_function === "ADVISOR" && work_status == 1) ||
      work_status == 3
    ) {
